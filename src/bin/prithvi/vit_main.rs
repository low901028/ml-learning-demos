...
mod prithvi {
    ...
    use tiff::decoder::{Decoder as TiffReader, DecodingResult};
    use std::path::Path;
    ...
    impl GeoTiffLoader {
        ...
        pub fn load<P: AsRef<std::path::Path>>(&self, path: P) -> Result<Tensor> {
            ...
            let data: Vec<f32> = img.chunks_exact(6)  // 处理6通道数据
                .flat_map(|chunk| {
                    chunk.iter().map(|&v| v as f32)
                })
                .collect();

            let tensor = Tensor::from_vec(data, (6, h as usize, w as usize), &self.device)?
                .to_dtype(DType::F32)?;
    ...
    impl PrithviViT {
        pub fn from_pretrained(path: &Path, device: &Device) -> Result<Self> {
        ...
        fn new(
            ...
            decoder_depth: usize,
            decoder_num_heads: usize,
            mlp_ratio: f32,
            mask_ratio: f32,
        ) -> Result<Self> {
            let num_patches = (img_size / patch_size[1]) * (img_size / patch_size[2]);
            let patch_dim = in_chans * patch_size[0] * patch_size[1] * patch_size[2];

            // Encoder部分
            let patch_embed = linear(patch_dim, embed_dim, vb.pp("decoder.decoder_embed"))?;
            let pos_embed = vb.get((1, num_patches + 1, embed_dim), "decoder.decoder_pos_embed")?;

            let mut encoder_layers = Vec::with_capacity(depth);
            for i in 0..depth {
    ...
    mod tests {
        ...
        fn test_model_loading() -> Result<()> {
            ...
            let model = PrithviViT::from_pretrained(Path::new(MODEL_PATH), &device)?;
        ...
        fn test_forward_pass() -> Result<()> {
            let device = Device::Cpu;
            let model = PrithviViT::from_pretrained(Path::new(MODEL_PATH), &device)?;
            let loader = GeoTiffLoader::new(device.clone());
        ...
        fn test_model() -> Result<()> {
            use super::{GeoTiffLoader, PrithviViT};
